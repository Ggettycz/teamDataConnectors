...
const connectorTypes = '';//DataExport'; //"StatsCreate"';
...
(async function(){
    ...
    for (const row of rows) {
        ...
        let runResult = false;
        logMessage(`--------- start of ${connectorType} for ${connectorName}  uuid: ${uuid} `, 'INFO')
        if (connectorType == 'StatsCreate') {
            const connector = new StatsCreate(link, uuid, configuration);
            runResult = await connector.runConnector();
        } else if (connectorType == 'DataExport') {
            configuration.toDate='2024-02-24';
            const connector = new FileExport(link, uuid, connectorName, configuration);
            runResult = await connector.runConnector();
        } else {
            logMessage(`Nothing to run for "${connectorName}"  uuid: ${uuid}.`,'WARN');
            continue;
        }
        if (runResult) {
